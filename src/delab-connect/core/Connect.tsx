...
import { isWalletInfoInjected, TonConnect, Wallet } from '@tonconnect/sdk';
import { WalletInfoRemote } from '@tonconnect/sdk/lib/types/models/wallet/wallet-info';
...
class DeLabConnect {
    ...
    private _connector2: TonConnect;

    constructor(appUrl: string, appName: string, connector: TonConnect, hostNameTonkeeper = 'https://tegro.finance', network: DeLabNetwork = 'mainnet') {
        this._appUrl = appUrl;
        ...
        this._connector2.onStatusChange((wallet) => {
            if (wallet) {
                this.onConnectTonkeeper(wallet);
            }
        }, console.error);
    ...
    private static async sendTransactionToncoinWallet(transaction: DeLabTransaction): Promise<boolean> {
        const sendData = {
    ...
    private async restoreTonkeeper() {
        this._connector2.restoreConnection();
        for (let i = 0; i < 1000; i++) {
            if (this._connector2.connected) {
                this._address = new Address(this._connector2.account!.address).toString('base64', { bounceable: true });
    ...
    public async onConnectTonkeeper(wallet: Wallet) {
        const address = new Address(wallet.account.address).toString('base64', { bounceable: true });
        console.log(address);
    ...
    public async handleConnectTonkeeper() {
        const walletsList = await this._connector2.getWallets();

        const keeper = walletsList[0] as WalletInfoRemote;

        const tonkeeperConnectionSource = {
            universalLink: keeper.universalLink,
            bridgeUrl: keeper.bridgeUrl,
        };

        const universalLink = this._connector2.connect(tonkeeperConnectionSource);
        ...
        if (embeddedWallet) {
            this._connector2.connect({ jsBridgeKey: embeddedWallet.jsBridgeKey });
        }
    }
    ...
    public disconnect(): boolean {
        ...
        this.clearStorage();
        try { this._connector2.disconnect(); } catch { }
